...
const BlogCard = ({ title, author, description }) => {
  return (
    <div className="group card relative bg-white border-b-2 border-orange-500 shadow-md rounded-2xl p-6 hover:shadow-lg hover:border-[1] transition-shadow duration-300">
      <h1 className="text-2xl font-bold text-gray-900 mb-2 ">{title}</h1>
      <h3 className="text-sm text-gray-500 mb-4 text-red-600">By: {author}</h3>
      <p className="text-gray-600 leading-relaxed">{description}</p>
      <button className=" group/bookmark absolute top-6 right-12 text-red-300 cursor-pointer ">
        <FaBookmark className="w-5 h-5 transform transition-transform duration-300 hover:scale-125 hover:text-red-400" />
